...
import models, schema as schemas
...
class UserRepo:
    async def create(db: Session, user: schemas.UserSchema):
        # print(user.dict())
        db_user = models.User(full_name=user.fullname,username=user.email,password=user.password)
        db.add(db_user)
        db.commit()
        db.refresh(db_user)
        return db_user

    async def getAllUser(db: Session,skip: int = 0, limit: int = 100):
        # print(user.dict())
        return db.query(models.User).offset(skip).limit(limit).all()

    async def login(db: Session,login:schemas.UserLoginSchema):
        login=db.query(models.User).filter_by(username=login.username,password=login.password).first()
        return login
    async def fetch_user_by_id(user_id:int,db: Session):
